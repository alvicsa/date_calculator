...
if __name__ == '__main__':
    ...
    while True:
        ...
        # Unpack user input into day, month and year
        f_d, f_m, f_y = first_date.split(sep='/')
        s_d, s_m, s_y = second_date.split(sep='/')

        # Create Date objects from the Date Class and convert inputs into integers
        dc_f = Date(int(f_d), int(f_m), int(f_y))
        dc_s = Date(int(s_d), int(s_m), int(s_y))

        if 2999 > dc_f.y < 1901 or 2999 > dc_s.y < 1901:
            print("Dates must be between 01/01/1901 and 31/12/2999. Try again!")
        else:
            # Print resulting date difference
            print(f'{first_date} - {second_date} =', getDifference(dc_f, dc_s), "days")
            break
